...
For the module config files, the ini format because it extremely fast to parse with PHP's parse_ini_file(), notably faster than xml, and there is generally no sensitive info associated with conf.ini files.

See the [php manual](http://www.php.net/manual/en/function.parse-ini-file.php) for an example ini-file.
...
Within any manager, $this->conf is inherited from the SGL_Manager parent class:
