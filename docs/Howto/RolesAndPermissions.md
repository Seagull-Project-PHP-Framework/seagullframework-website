...
That's right, keep in mind that $dao->retrieveAllPerms() is called by the SGL_Process_SetupPerms() task which builds the global perms array on the first request, then caches the array to disk for subsequent calls providing you have caching enabled.
...
Sometimes in your module you may need to check a custom permission, eg MEMBERSHIP_GOLD. Based on the results you may  choose a different template to show/hide extra functionality according to users permissions, or a non-default theme. Permission can be checked with a static call to the SGL_Session method
...
Implementing this modification means you can drop the user_permission table.
